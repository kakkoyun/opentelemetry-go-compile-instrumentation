...
**2. Metrics phone home by default.** The meter provider is set up unconditionally via autoexport (`pkg/runtime/setup.go:176-197` — no endpoint or enable check guards it, unlike `setupTraceProvider`), so every otelc binary — including one with zero matched instrumentations — exports runtime metrics to `localhost:4318` on the standard interval. Observed directly: 4 connections to a listener on :4318 from an 8-second hello-world run with a shortened export interval. Trace and metric defaults therefore point in opposite directions: traces need explicit opt-in, metrics are opt-out (and there's no documented opt-out besides `OTEL_METRICS_EXPORTER=none`).

**3. `OTEL_SDK_DISABLED=true` does nothing.** The SDK, runtime metrics, and all hooks initialize regardless. `pkg/runtime/otel_setup.go:31` documents `OTEL_SDK_DISABLED` as a recognized variable in `SetupOTelSDK`'s own doc comment, but `SetupOTelSDK` (`otel_setup.go:32-38`) never reads it. The spec defines this variable as the kill switch for the whole SDK.
...
No fix is in progress as of this audit; this remains an open decision. It is one of the four items this readiness review treats as a true compat-sense v1 blocker (see `docs/readiness/README.md`), alongside the rule-schema freeze, the distribution/config model, and the dependency policy.

Evidence: [a5-summary.txt](../a5-summary.txt), [a5-otlp-listener3.log](../a5-otlp-listener3.log).
