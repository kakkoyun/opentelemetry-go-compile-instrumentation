...
**Recommendation (non-binding).** Option B is already in flight upstream (PR #655) and has no real downside; it should land regardless of what else is decided here. Options A, C, and D are genuine policy choices with user-visible tradeoffs and are the ones that actually need SIG discussion before the v1 tag, since whichever way they're decided becomes the compatibility contract this ADR exists to fix in place.
