...
func (s *StateManager) Commit() error {
	...
	if err = os.MkdirAll(filepath.Dir(f), 0o755); err != nil {
		return ex.Wrapf(err, "failed to create state directory for %s", f)
	}

	tmp := f + ".tmp"
	if err = os.WriteFile(tmp, bs, 0o600); err != nil {
		return ex.Wrapf(err, "failed to write state to file %s", tmp)
	}
	if err = os.Rename(tmp, f); err != nil {
		return ex.Wrapf(err, "failed to move state file into place at %s", f)
	}
