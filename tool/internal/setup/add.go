...
func genVarDecl(matched []*rule.InstFuncRule) []dst.Decl {
	...
	for i, m := range matched {
		...
		// var _printstack%d = func (bt []byte){ _otel_log.Printf("%s", string(bt)) }
		// The explicit "%s" format keeps the generated code vet-clean: go test
		// runs vet by default, and a non-constant format string fails the
		// printf check (and would misinterpret any % in the stack trace).
		// Build: string(bt)
		stringCall := &dst.CallExpr{
			Fun:  ast.Ident("string"),
			Args: []dst.Expr{ast.Ident("bt")},
		}
		// Build: _otel_log.Printf("%s", string(bt))
		printfCall := &dst.CallExpr{
			Fun:  ast.SelectorExpr(ast.Ident("_otel_log"), "Printf"),
			Args: []dst.Expr{ast.StringLit("%s"), stringCall},
		}
		// Build: func (bt []byte) { _otel_log.Printf("%s", string(bt)) }
...
func (sp *SetupPhase) addDeps(ctx context.Context, matched []*rule.InstRuleSet, packagePath, pkgName string) error {
	if pkgName == "" {
		// File-target builds load as command-line-arguments; they are always
		// package main.
		pkgName = "main"
	}
	...
	// Build the ast
	root := buildOtelcRuntimeAst(pkgName, append(importDecls, varDecls...))
