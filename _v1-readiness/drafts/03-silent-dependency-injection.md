...
Separately, all embedded modules declare `go 1.25.0`, which means otelc requires every target app to be on go >= 1.25 (apps on 1.24 currently fail with an unrelated-looking error — see the companion "cannot replace" issue). Go 1.24 is still a supported upstream release.
...
Evidence: `a2-gomod-during-build.txt` (full mid-build go.mod of a hello-world), `a2-static-version-audit.txt`.
